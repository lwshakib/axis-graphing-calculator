...
export function ScientificWorkspace({ initialData, sessionId }: ScientificWorkspaceProps) {
  const mfRef = useRef<MathfieldElement>(null);
  const [mounted, setMounted] = useState(false);
  const [equation, setEquation] = useState("");
  const [result, setResult] = useState<any>(null);
  const [error, setError] = useState<string | null>(null);
  const [variables, setVariablesList] = useState<Record<string, string>>(initialData?.variables || {});
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [history, setHistoryList] = useState<{expr: string, res: any}[]>(initialData?.history || []);
  const [title, setTitle] = useState(initialData?.title || "Untitled Scientific");
