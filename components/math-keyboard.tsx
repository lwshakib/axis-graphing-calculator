    ...
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-t border-border shadow-2xl transition-all duration-300 animate-in slide-in-from-bottom-full">
      <div className="max-w-4xl mx-auto p-4">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Math Keyboard</h3>
          ...
          {KEYS.flat().map((key) => (
            <Button
              ...
              className={cn(
                ...
                ['/', '*', '-', '+', '='].includes(key) ? "bg-primary/10 text-primary border-primary/20" : 
                ['sin', 'cos', 'tan', 'sqrt', 'log', 'ln', 'abs'].includes(key) ? "bg-indigo-500/10 text-indigo-500 border-indigo-500/20" :
                "bg-secondary/50"
              )}
              onClick={() => onInput(key)}
            >
