...
interface SavedSession {
  id: string;
  title: string;
  type: string; // 'graph' | 'calculator' | 'scientific' | '3d'
  updatedAt: string;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  data: any;
}

/** Icon mapping for visual cues based on session type. */
...
/** Style mapping for session type badges. */
...
export default function DashboardPage() {
  ...
  /** Authenticate: Protect route and redirect guests to Not Found/Login. */
  useEffect(() => {
  ...
  /** Data Fetching: Retrieve all user-stored sessions from the API. */
  ...
  /** Delete Handler: Removes a session record permanently. */
  ...
  /** Client-side filtering of session list based on Type (Tabs) and Title (Search). */
  ...
  // Handle generic screen-wide loading state
  if (isPending) {
    return (
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          >
            <TabsList className="bg-muted/50 p-1 rounded-xl h-14">
              {["all", "graph", "3d", "calculator"].map((tab) => (
                <TabsTrigger
                  key={tab}
                  value={tab}
