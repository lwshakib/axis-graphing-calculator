...
const features = [
  {
    ...
    description: "High-performance interactive engine supporting function plotting, derivatives, and infinite exploration.",
    href: "/graph",
    icon: LayoutGrid,
    variant: "blue",
  },
  {
    title: "3D Mapping",
    description: "Visualize multivariate calculus and vector fields. Plot complex surfaces with dynamic lighting.",
    icon: Box,
    href: "/3d",
    variant: "teal",
  },
  {
    ...
    description: "Natural math input with support for matrices, symbolic simplification, and numerical integration.",
    href: "/scientific",
    icon: FunctionSquare,
    variant: "purple",
  },
  {
    title: "Standard Calc",
    description: "A sleek, tactile interface for rapid daily calculations. Balanced for speed and focus.",
    href: "/calculator",
    icon: Calculator,
    variant: "orange",
  },
...
export default function Home() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-background text-foreground transition-colors duration-500 overflow-x-hidden">
      {/* Hero Section */}
      <section className="relative h-[95vh] flex items-center justify-center pt-20 overflow-hidden">
        ...
        >
          <div className="max-w-3xl mx-auto text-center space-y-8">
            ...
            >
              <Badge variant="secondary" className="px-4 py-1.5 rounded-full bg-primary/5 text-primary border-primary/10 font-medium tracking-wide">
            ...
            >
              Precise. <span className="text-muted-foreground/40">Effortless.</span>
            </motion.h1>
            ...
            >
              ...
              high-fidelity visualization and symbolic mastery.
            </motion.p>
            ...
            >
              <Button size="lg" className="h-14 px-8 rounded-full font-semibold text-base gap-2 group shadow-lg shadow-primary/10 hover:shadow-primary/20 transition-all active:scale-95" asChild>
                <Link href="/graph">
                  Enter Workspace
                  <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" />
                </Link>
              </Button>
              <Button size="lg" variant="ghost" className="h-14 px-8 rounded-full font-semibold text-base border border-transparent hover:border-border transition-all active:scale-95" asChild>
                <Link href="/dashboard">
                  My Sessions
                </Link>
              </Button>
        ...
        >
          <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground/40 text-center">Explore AXIS</span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={i} className="text-center group">
                <div className="text-2xl md:text-3xl font-semibold tracking-tight mb-0.5 group-hover:text-primary transition-colors">{stat.value}</div>
                <div className="text-[10px] uppercase tracking-[0.2em] font-bold text-muted-foreground/40">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-32 bg-background">
        <div className="container mx-auto px-6">
          <div className="max-w-2xl mb-24">
            <h2 className="text-3xl md:text-5xl font-bold mb-6 tracking-tight leading-none">Engineered for Mastery.</h2>
            <p className="text-lg text-muted-foreground font-normal leading-relaxed">
              Combining symbolic computation with high-performance graphics to create the ultimate playground for mathematical exploration.
            </p>
          </div>
                  ...
                  <Card className="h-full border-border/40 bg-card/50 hover:bg-accent/30 hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 rounded-3xl overflow-hidden">
                    <CardHeader className="pb-4">
                      <div className={cn(
                        ...
                        feature.variant === "blue" && "bg-blue-500/10 text-blue-500",
                        feature.variant === "teal" && "bg-emerald-500/10 text-emerald-500",
                        feature.variant === "purple" && "bg-purple-500/10 text-purple-500",
                        feature.variant === "orange" && "bg-orange-500/10 text-orange-500",
                      )}>
              ...
              <div className="space-y-10">
                {[
                  ...
                  <div key={i} className="flex gap-6 group">
                    ...
                    <div>
                      <h4 className="font-bold text-lg mb-1.5 tracking-tight">{item.title}</h4>
              ...
              <div className="absolute bottom-8 left-8 right-8 p-5 rounded-2xl border border-border/40 bg-background/80 backdrop-blur-md shadow-lg flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="space-y-0.5">
                    <span className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground block">Sampling</span>
                    <span className="text-xs font-mono font-medium">1.2e-16 @ 64bit</span>
                  </div>
                </div>
      ...
      <section className="py-32 border-t border-border/40">
        <div className="container mx-auto px-6 text-center max-w-2xl">
          <h2 className="text-3xl md:text-5xl font-bold tracking-tight mb-8">Ready to explore?</h2>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
           <Button size="lg" className="h-14 px-10 rounded-full font-bold text-base" asChild>
              <Link href="/graph">Start Graphing</Link>
            </Button>
            <Button size="lg" variant="outline" className="h-14 px-10 rounded-full font-bold text-base border-border/60" asChild>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-12 mb-20">
            <div className="space-y-6">
              ...
              <p className="max-w-xs text-base text-muted-foreground leading-relaxed">
                Reimagining tools of discovery for the next generation of scientific explorers.
              </p>
              <div className="flex gap-4">
                {[Twitter, Github, Linkedin].map((Icon, i) => (
                  <Link key={i} href="#" className="w-10 h-10 rounded-full border border-border flex items-center justify-center hover:bg-muted transition-all active:scale-90">
                    <Icon size={18} />
                  </Link>
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-16">
              <div className="space-y-6">
                <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground/50">Ecosystem</h4>
                <ul className="space-y-4">
                  {["Graph", "3D Plotter", "Scientific", "Calculator"].map(item => (
                    <li key={item}>
                      <Link href={`/${item.toLowerCase().replace(' ', '')}`} className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">{item}</Link>
                    </li>
                  ))}
                </ul>
              </div>
              <div className="space-y-6">
                <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground/50">Resources</h4>
                <ul className="space-y-4">
                  {["Docs", "Release", "Community", "Privacy"].map(item => (
                    <li key={item}>
                      <Link href="#" className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">{item}</Link>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="pt-8 border-t border-border/20 flex flex-col sm:flex-row justify-between items-center gap-6">
            ...
            <div className="flex gap-8">
              {["Privacy", "Terms", "Status"].map(item => (
                <Link key={item} href="#" className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/40 hover:text-primary transition-colors">{item}</Link>
              ))}
            </div>
          </div>
        </div>
