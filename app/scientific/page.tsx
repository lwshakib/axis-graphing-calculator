...
import { evaluateMath, formatResult, setVariable, derivative, simplify, integrate } from "@/lib/math-parser";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function ScientificCalculatorPage() {
  ...
  const insertAtCursor = (content: string, options: { latex?: boolean } = {}) => {
    if (mfRef.current) {
      if (options.latex) {
  ...
  const handleCalculate = () => {
    ...
    // Get ascii-math for mathjs evaluation
  ...
  const handleDerivative = () => {
    ...
    try {
      ...
      // We don't calculate immediately, just show the derivative
    } catch (e: any) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] w-full flex items-center justify-center p-4 bg-gradient-to-br from-indigo-50 to-blue-100 dark:from-zinc-950 dark:to-zinc-900 overflow-x-hidden">
      <div className="w-full max-w-5xl bg-white dark:bg-zinc-900 rounded-[2.5rem] shadow-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 flex flex-col lg:flex-row transition-all duration-500">
        ...
        {/* Left Sidebar: Variables & History */}
        <div className="lg:w-1/4 bg-zinc-50 dark:bg-zinc-900/50 p-6 border-b lg:border-b-0 lg:border-r border-zinc-200 dark:border-zinc-800 flex flex-col gap-8">
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar pr-2">
              {Object.keys(variables).length === 0 ? (
                <div className="bg-zinc-100/50 dark:bg-zinc-800/30 p-3 rounded-xl border border-dashed border-zinc-200 dark:border-zinc-700">
                  <p className="text-zinc-400 text-[10px] text-center italic">Type "A = 10" to save</p>
                </div>
              ) : (
                Object.entries(variables).map(([name, val]) => (
                  <button key={name} className="w-full flex justify-between items-center bg-white dark:bg-zinc-800 p-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm hover:ring-2 hover:ring-indigo-500/20 transition-all text-left" onClick={() => insertAtCursor(name)}>
                    <span className="font-black text-indigo-600 dark:text-indigo-400">{name}</span>
                    <span className="text-[10px] text-zinc-500 truncate ml-2 max-w-[80px] font-mono">{val}</span>
                  </button>
                ))
              )}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar pr-2">
              {history.map((item, i) => (
                <div key={i} className="group p-2.5 rounded-xl bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 hover:border-indigo-200 transition-all cursor-pointer" onClick={() => { if (mfRef.current) mfRef.current.value = item.expr; }}>
                  <p className="text-[10px] text-zinc-400 truncate mb-0.5">{item.expr}</p>
                  <p className="text-xs font-bold text-zinc-700 dark:text-zinc-200 truncate">{formatResult(item.res)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-auto">
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button variant="outline" className="w-full gap-2 rounded-2xl h-12 border-2 border-dashed border-zinc-200 dark:border-zinc-700 text-zinc-500 hover:text-indigo-600 hover:border-indigo-600/50 transition-all">
              ...
              <DialogContent className="sm:max-w-[425px] bg-white dark:bg-zinc-900 rounded-[2rem] border-none shadow-2xl p-8">
                <DialogHeader>
                  <DialogTitle className="text-2xl font-black text-zinc-900 dark:text-white">Create Matrix</DialogTitle>
                </DialogHeader>
                <div className="py-6 space-y-6">
                  <div className="flex gap-4 items-center">
                    <div className="flex flex-col gap-1.5 flex-1">
                      <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest pl-1">Variable Name</label>
                      <Input placeholder="A" value={matrixVarName} onChange={(e) => setMatrixVarName(e.target.value.toUpperCase())} className="h-12 rounded-xl text-center font-black text-xl text-indigo-600 border-2 focus:border-indigo-600" />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest pl-1">Dimensions</label>
                      <div className="flex items-center gap-2">
                        <Input type="number" value={matrixRows} onChange={(e) => resizeMatrix(parseInt(e.target.value) || 1, matrixCols)} className="w-16 h-12 text-center rounded-xl font-bold" />
                        <span className="text-zinc-300 font-bold">×</span>
                        <Input type="number" value={matrixCols} onChange={(e) => resizeMatrix(matrixRows, parseInt(e.target.value) || 1)} className="w-16 h-12 text-center rounded-xl font-bold" />
                      </div>
                    </div>
                  </div>
                  <div className="grid gap-2 p-6 bg-zinc-50 dark:bg-zinc-800/50 rounded-2xl border-2 border-zinc-100 dark:border-zinc-700 max-h-64 overflow-auto custom-scrollbar" style={{ gridTemplateColumns: `repeat(${matrixCols}, 1fr)` }}>
                    {matrixData.map((row, r) => row.map((val, c) => (
                      <Input key={`${r}-${c}`} value={val} onChange={(e) => updateMatrixCell(r, c, e.target.value)} className="h-10 text-center text-sm p-1 rounded-lg border-zinc-200" />
                    )))}
                  </div>
                  <Button onClick={createMatrix} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl h-14 font-black text-lg shadow-xl shadow-indigo-600/20 active:scale-95 transition-all">Insert Matrix</Button>
                </div>
              </DialogContent>
            </Dialog>
          </div>
        ...
        <div className="flex-1 p-8 flex flex-col gap-6">
          {/* Display Area: Natural Input + Glass Rendering */}
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between px-2">
               <span className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em]">Input Equation</span>
               <div className="flex gap-2">
                 <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button className="text-zinc-400 hover:text-indigo-500 transition-colors"><Info size={14} /></button>
                      </TooltipTrigger>
                      <TooltipContent className="bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 rounded-xl p-3 shadow-xl">
                        <p className="text-xs">Usa <b>LaTeX</b> or natural math. <br/>Click symbols below to insert.</p>
                      </TooltipContent>
                    </Tooltip>
                 </TooltipProvider>
               </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-3 bg-zinc-50 dark:bg-zinc-800/30 p-4 rounded-[2rem] border border-zinc-100 dark:border-zinc-800">
                <CalcButton onClick={clear} className="text-red-500 bg-red-50/50 dark:bg-red-950/10 border-red-100 hover:bg-red-100 hover:text-red-600">AC</CalcButton>
                <CalcButton onClick={deleteLast} className="bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200"><MoveLeft size={20}/></CalcButton>
                <CalcButton onClick={() => insertAtCursor('%')} className="bg-zinc-100 dark:bg-zinc-800 text-zinc-600">%</CalcButton>
                <CalcButton onClick={() => insertAtCursor('/')} className="bg-indigo-600 text-white shadow-lg shadow-indigo-600/20 border-none hover:bg-indigo-700">÷</CalcButton>
                ...
                <CalcButton onClick={() => insertAtCursor('0')} className="col-span-1">0</CalcButton>
                <CalcButton onClick={() => insertAtCursor('.')}>.</CalcButton>
                <CalcButton onClick={() => insertAtCursor('=')} className="bg-zinc-200 dark:bg-zinc-700 text-indigo-600 dark:text-indigo-400 font-black text-2xl border-none hover:bg-zinc-300">=</CalcButton>
                <CalcButton onClick={handleCalculate} className="bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 border-none hover:bg-emerald-600">
                  <Equal size={28} />
                </CalcButton>
              </div>
            </div>
      ...
      <style jsx global>{`
        ...
        /* Hide default mathlive keyboard button if it appears */
...
function SciFuncButton({ children, onClick, className }: { children: React.ReactNode; onClick?: () => void; className?: string }) {
  return (
    <button
      onClick={onClick}
      className={cn(
